...
def verificaPosti(cursor, idEvento, numeroPrenotati):
    ...
    cursor.execute("SELECT SUM(numeroPrenotati) FROM prenotazioni WHERE FKidEvento = %s", (idEvento,))
    postiPrenotati = cursor.fetchone()[0] or 0  

    # Recupera il numero massimo di posti disponibili per l'evento
    cursor.execute("SELECT numeroPosti FROM eventi WHERE idEvento = %s", (idEvento,))
...
def login():
    ...
    try:
        ...
        cursor.execute("SELECT idUtente,nome,password FROM utenti WHERE email = %s", (email,))
...
def inserisciPrenotazione():
    ...
    try:
        ...
        if verificaPosti(cursor,idEvento,numeroPrenotati):
            ...
            cursor.execute("INSERT INTO prenotazioni (FKidEvento,FKidUtente,numeroPrenotati,noteEvento) VALUES (%s, %s, %s, %s)", (idEvento, idUtente, numeroPrenotati, note))
            connection.commit()
        else:
...
def modificaPrenotazione(idPrenotazione):
    ...
    try:
        ...
        if verificaPosti(cursor,idEvento,numeroPrenotati):
            ...
            cursor.execute("UPDATE prenotazioni set numeroPrenotati = %s, noteEvento = %s where idPrenotazione = %s", (numeroPrenotati, note, idPrenotazione))
            connection.commit()
        else:
            return jsonify({'error': 'Numero di posti non disponibile per l\'evento'}), 400
...
def cancellaPrenotazione(idPrenotazione):
    try:
        ...
        cursor.execute("DELETE FROM prenotazioni WHERE idPrenotazione = %s", (idPrenotazione,))
        connection.commit()
        cursor.close()
        connection.close()
...
def verificaPrenotazione():
    ...
    try:
        ...
        cursor.execute("SELECT * from prenotazioni where FKidUtente = %s and FKidEvento = %s", (idUtente,idEvento))
        result = cursor.fetchone()
